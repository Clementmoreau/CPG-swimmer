function out=generate_fig_switch10_draw(tps, traj, params)
...
for i=timeseq
    [X,Y,TH]=coordinates_filament(traj(i,:),params);
    plot(X+15*(i-1)/length(tps),Y,'LineWidth',2) 
end
    ...
    for i=timeseq
    [X,Y,TH]=coordinates_filament(traj(i,:),params);    
    plot(X(1)+15*(i-1)/length(tps),Y(1),'o','MarkerSize',3,...
...
dpi = '-r400';
saveas(fig5,'fig_switch10_crawl_angle','epsc')
saveas(fig6,'fig_switch10_crawl_phase','epsc')
%saveas(fig7,'fig7','epsc') 
saveas(fig8,'fig_switch10_crawl_shape','epsc') 
