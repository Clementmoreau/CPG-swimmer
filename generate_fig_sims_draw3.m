...
set(gcf, 'Position',  [1, 640, figsize*1, figsize*0.5])  
    hold on
    timeseq=1:50:length(tps);%floor(0.8*length(tps)):6:length(tps);
for i=timeseq
    [X,Y,TH]=coordinates_filament(traj(i,:),params);
...
fig5=figure(5);clf;
set(gcf, 'Position',  [figsize, 640, figsize*1, figsize*0.5])    
...
fig6=figure(6);clf;
set(gcf, 'Position',  [figsize, 300, figsize*1, figsize*0.5])    
...
dpi = '-r400';
saveas(fig5,'fig_sims_crawl_angle','epsc')
saveas(fig6,'fig_sims_crawl_phase','epsc')
%saveas(fig7,'fig7','epsc') 
saveas(fig8,'fig_sims_crawl_shape','epsc') 
