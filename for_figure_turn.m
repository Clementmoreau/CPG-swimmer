...
% Plot some snapshots
col = colormap(othercolor('Paired6',i_fin-i_ini+1));

for i=i_ini:iframe:i_fin%i=length(tps)-1000:10:length(tps)
    [X,Y,TH]=coordinates_filament(traj(i,:),params);
    plot(X,Y,'LineWidth',5,'Color',col(i-i_ini+1,:))    
end
    ...
    exportgraphics(gcf,'fig_turn_1.pdf','ContentType','vector')
...
set(gcf, 'Position',  [1,1,309,134])    
    ...
    exportgraphics(gcf,'fig_turn_2.pdf','ContentType','vector')
...
exportgraphics(gcf,'fig_turn_3.pdf','ContentType','vector')
