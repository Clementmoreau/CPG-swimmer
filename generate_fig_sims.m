function out=generate_fig_sims()
...
generate_fig_sims_draw1(tps, traj, params);

% fig_sims_swimH
clear tps traj
Sp=6; params.Sp=Sp;
tau=12;params.tau=tau;
sigma_amp = 8;sigma = @(t) sigma_amp;params.sigma=sigma;
[tps, traj]=generate_fig_main(params, inits, opts);
generate_fig_sims_draw2(tps, traj, params);

% fig_sims_crawl
clear tps traj
Sp=6; params.Sp=Sp;
tau=8;params.tau=tau;
gamma=1/70;params.gamma=gamma;
sigma_amp = 12;sigma = @(t) sigma_amp;params.sigma=sigma;
coup = 0.1; coupR = coup; 
params.coup=coup;params.coupR=coupR;
T=8; opts.T=T;
z0 = [0;0;0*pi;0*pi*ones(N-1,1)];
inits = [z0;p0];
[tps, traj]=generate_fig_main(params, inits, opts);
generate_fig_sims_draw3(tps, traj, params);
